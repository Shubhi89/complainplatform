...
const ConsumerDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8"> {/* Responsive Padding */}
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {complaints.map((complaint) => (
              <div key={complaint._id} className="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition cursor-pointer">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                  <div className="w-full">
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                      <span className="font-mono text-xs sm:text-sm text-gray-400">#{complaint.complaintId}</span>
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${getStatusColor(complaint.status)}`}>
                        {complaint.status}
                      </span>
                    </div>
                    <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-1">{complaint.title}</h3>
