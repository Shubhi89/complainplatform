...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (error.response?.status === 403 && (error.response?.data?.code === 'SECRET_REQUIRED' || error.response?.data?.requiresSecret)) {
  ...
  const handleApprove = async (id: string, action: 'APPROVED' | 'REJECTED') => {
    ...
    try {
      await axios.post('/api/admin/approve-business', { profileId: id, action });
      // Optimistic Update
  ...
  if (loading && !isAuthorized) return <div className="p-10 text-center">Loading Dashboard...</div>;
  if (!isAuthorized && errorMsg) return <div className="p-10 text-center text-red-600">{errorMsg}</div>;
       ...
       <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            ...
            <p className="text-gray-500">Platform performance & pending actions</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex items-center justify-between hover:shadow-md transition">
             <div>
                <p className="text-gray-500 text-sm font-medium mb-1">Total Consumers</p>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex items-center justify-between hover:shadow-md transition">
             <div>
                <p className="text-gray-500 text-sm font-medium mb-1">Total Businesses</p>
                <h2 className="text-3xl font-bold text-gray-900">{stats?.totalBusinesses || 0}</h2>
             </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex items-center justify-between hover:shadow-md transition">
             <div>
                <p className="text-gray-500 text-sm font-medium mb-1">Verified Businesses</p>
                <div className="flex items-end gap-2">
                   <h2 className="text-3xl font-bold text-gray-900">{stats?.verifiedBusinesses || 0}</h2>
            ...
            <div className="divide-y divide-gray-100">
              {verifications.length === 0 ? (
                 ...
                 verifications.map(req => (
                    <div key={req._id} className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-gray-50 transition">
                       
                       {/* Business Info */}
                       <div>
                          <div className="flex items-center gap-2 mb-1">
                             <h3 className="font-bold text-lg text-gray-900">{req.companyName}</h3>
                             <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded border border-gray-200">{req.industry}</span>
                          </div>
                          <p className="text-sm text-gray-500 mb-2">{req.user?.email}</p>
